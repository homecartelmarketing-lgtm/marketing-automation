"""Scrape Newest Active Chandeliers from Akeneo into 1 Product, 3 Styles Feed Table.

Table ID: tblrlfqBGe5EjS5PI
...
Usage:
    ...
    python "1 Product 3 Styles Feed/3_Scrape_Akeneo_Chandeliers.py" --execute

    # 3. Mass-scrape N products:
    python "1 Product 3 Styles Feed/3_Scrape_Akeneo_Chandeliers.py" --execute --max-items 5
"""
...
DEFAULT_TABLE_ID = (
    os.getenv("AIRTABLE_TABLE_ID_CHANDELIER_1_PRODUCT_3_STYLES", "").strip()
...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(
        description="Scrape active Chandeliers for 1 Product, 3 Styles Feed (Newest First, Cross-Table Dedup)"
    )
    parser.add_argument(
        "--execute",
        action="store_true",
...
def main():
    ...
    print("\n" + "=" * 68)
    print(" HomeCartel - Akeneo Chandelier Scraper for 1 Product, 3 Styles Feed")
