"""Scrape Newest Active Floor Lamps from Akeneo into Before & After Reel Table.

Table ID: tbl2VoWOt7sSut4E2
Category: Floor Lamps (Modern)
Krea Moodboard ID: b1641228-beec-4823-8d01-1de3eec8410d
Krea Prompt: "Generate me a bedroom that have beside a floor lamp"
...
Usage:
    ...
    python "Before and After Reel/3_Scrape_Akeneo_Floor_Lamps.py"

    # 2. Scrape 1 product and save to Airtable (Execute):
    python "Before and After Reel/3_Scrape_Akeneo_Floor_Lamps.py" --execute

    # 3. Scrape N products:
    python "Before and After Reel/3_Scrape_Akeneo_Floor_Lamps.py" --execute --max-items 3
"""
...
DEFAULT_TABLE_ID = os.getenv("AIRTABLE_TABLE_ID_FLOORLAMP_DAY_AND_NIGHT_REEL", "").strip() or "tbl2VoWOt7sSut4E2"
...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(
        description="Scrape active Floor Lamps for Before & After Reel (Newest First, Cross-Table Dedup)"
    )
    parser.add_argument(
        "--execute",
        action="store_true",
...
def main():
    ...
    print("\n" + "=" * 64)
    print(" HomeCartel - Akeneo Floor Lamp Scraper for Before & After Reel")
    print(f" Target Table ID: {args.table_id}")
    print(f" Category: Floor Lamps | Style: {args.style}")
    print(f" Mode: {mode}")
