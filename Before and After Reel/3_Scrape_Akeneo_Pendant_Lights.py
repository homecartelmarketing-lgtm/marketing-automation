"""Scrape Newest Active Pendant Lights from Akeneo into Before & After Reel Table.
...
Usage:
    ...
    python "Before and After Reel/3_Scrape_Akeneo_Pendant_Lights.py"

    # 2. Scrape 1 product and save to Airtable (Execute):
    python "Before and After Reel/3_Scrape_Akeneo_Pendant_Lights.py" --execute

    # 3. Scrape N products:
    python "Before and After Reel/3_Scrape_Akeneo_Pendant_Lights.py" --execute --max-items 3
"""
...
DEFAULT_TABLE_ID = os.getenv("AIRTABLE_TABLE_ID_PENDANT_LIGHTS_DAY_AND_NIGHT_REEL", "").strip() or "tbleUP86Kw36G8Hdw"
...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(
        description="Scrape active Pendant Lights for Before & After Reel (Newest First, Cross-Table Dedup)"
    )
    parser.add_argument(
        "--execute",
        action="store_true",
...
def main():
    ...
    print("\n" + "=" * 64)
    print(" HomeCartel - Akeneo Pendant Light Scraper for Before & After Reel")
