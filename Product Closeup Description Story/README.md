...
| **`3_Scrape_Akeneo_Cluster_Chandeliers.py`** | Kukuha ng mga active na Cluster Chandeliers (`tblnIOQVywHcTgAtv`) mula Akeneo na may cross-table deduplication. | Para mag-scrape ng Cluster Chandeliers. |
| **`3_Scrape_Akeneo_Table_Lamps.py`** | Kukuha ng mga active na Table Lamps (`tbl5S9JEHSrjrLwxA`) mula Akeneo na may cross-table deduplication. | Para mag-scrape ng Table Lamps. |
| **`3_Scrape_Akeneo_Wall_Lights.py`** | Kukuha ng mga active na Wall Lights (`tblYqudlgjYMNRROM`) mula Akeneo na may cross-table deduplication. | Para mag-scrape ng Wall Lights. |
...
### 2. Patakbuhin ang Buong Automation (End-to-End: Scrape -> 9:16 Story Card):
```bash
# Chandeliers (Default):
...
### 3. I-generate ang mga Pending Story Cards Lamang:
```bash
# Chandeliers (Default):
...
### 🎨 Model & Prompt Specs:
- **AI Model**: Fal AI GPT Image 2 (`openai/gpt-image-2`)
...
- **Execution Log**: `output/logs/fal_gpt_image2_product_description_logs.json`
