"""Scrape Cluster Chandeliers from Akeneo into Product Closeup w/ Description Table.

Table ID: tblnIOQVywHcTgAtv
Category: Cluster Chandeliers (Modern)
Row Capacity: 1 Product per Row + Product Closeup Description Layout Watermark

Usage:
    # Scrape 1 product (1 row):
    python "Product Closeup Description Story/3_Scrape_Akeneo_Cluster_Chandeliers.py"

    # Scrape N products (N rows):
    python "Product Closeup Description Story/3_Scrape_Akeneo_Cluster_Chandeliers.py" --count 3

    # Override Style or Table ID:
    python "Product Closeup Description Story/3_Scrape_Akeneo_Cluster_Chandeliers.py" --style modern --table-id tblnIOQVywHcTgAtv
"""
...
LABEL = "Cluster Chandeliers Product Closeup w/ Description"
DEFAULT_TABLE_ID = os.getenv("AIRTABLE_TABLE_ID_CLUSTER_CHANDELIER_PRODUCT_DESCRIPTION", "").strip() or "tblnIOQVywHcTgAtv"
...
def scrape_cluster_chandeliers(
    count: int = 1,
