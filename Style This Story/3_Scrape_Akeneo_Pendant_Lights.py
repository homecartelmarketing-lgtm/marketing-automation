"""Scrape Newest Active Pendant Lights from Akeneo into Style This Story Table.

Table ID: tblWdz71nULR0TZx7
...
Features:
1. Cross-Table Deduplication: Scans ALL tables in the Airtable base to verify if a pendant light already exists.
...
Usage:
    # 1. Scrape 1 new pendant light (Dry Run / Read-Only):
    python "Style This Story/3_Scrape_Akeneo_Pendant_Lights.py"

    # 2. Scrape 1 new pendant light and save to Airtable (Execute):
    python "Style This Story/3_Scrape_Akeneo_Pendant_Lights.py" --execute

    # 3. Scrape N new pendant lights:
    python "Style This Story/3_Scrape_Akeneo_Pendant_Lights.py" --execute --max-items 3
"""
...
DEFAULT_TABLE_ID = os.getenv("AIRTABLE_TABLE_ID_STYLE_THIS_PENDANT_LIGHTS", "").strip()
...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(
        description="Scrape active Pendant Lights (newest to oldest, cross-table deduplicated) for Style This Story"
    )
    parser.add_argument(
        "--execute",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Maximum number of new pendant light products to scrape (default: 1)
    parser.add_argument(
        "--starting-letter",
        default=None,
...
def scrape_pendant_lights_for_style_this(
    ...
    if not table_id:
        raise AutomationError(
            "Missing Airtable Table ID for Pendant Lights Style This.\n"
            "Please create the table in Airtable and set AIRTABLE_TABLE_ID_STYLE_THIS_PENDANT_LIGHTS in .env, "
    ...
    print("\n" + "=" * 64)
    print(" HomeCartel - Akeneo Pendant Light Scraper for Style This Story")
    print(f" Target Table ID: {table_id}")
    print(f" Category: Pendant Lights | Style: {style_code}")
    print(f" Mode: {mode}")
...
def main(argv=None) -> int:
    ...
    success = scrape_pendant_lights_for_style_this(
